...
Shader "MicroSplat/Example_Snow" {
   Properties {
   ...
   SubShader {
      ...
      // people like myself..
         ...
         #endif
         #if _SNOW && _USEGLOBALSNOWAGE
         float _Global_SnowAge;
         #endif
      ...
      fixed ComputeParticulateMask(float2 wuv, float4 parms)
      {
         ...
         fixed w1 = tex2D(_GlitterWind, wuv * 3 + speed * float2(1,0)).a;
         fixed w2 = tex2D(_GlitterWind, wuv + speed * float2(1.1,0)).a;
         return (w1 * w2);
      }

      fixed ProcessParticulateMask(inout fixed mask, float contrast)
      {
         mask = pow(mask, contrast);
      ...
      void DoWindParticulate(Input i, inout MicroSplatLayer o, Config config, half4 weights, float camDist, float3 worldNormalVertex, half snowWeight)
      {
         o.Occlusion *= saturate(abs(i.worldPos) + 2);
            ...
            half windBlend = 1.0;
            ...
            #if _WINDSHADOWS
               float2 offset = i.color.yx * _WindParticulateShadow.x;
               fixed shadowMask = ComputeParticulateMask(wuv + offset.xy, _WindParticulateParams);
               shadowMask *= _WindParticulateStrength * windBlend;
               ...
               windMask *= _WindParticulateStrength * windBlend;
               ...
               o.Occlusion *= windMask;
               o.Metallic *= windInvMask;
            #endif

            #if _SNOWSHADOWS
               float2 snowOffset = i.color.yx * _SnowParticulateShadow.x;
               fixed snowShadowMask = ComputeParticulateMask(wuv + offset.xy, _SnowParticulateParams) * _SnowParticulateStrength;
               snowShadowMask *= snowWeight;
               ...
               o.Occlusion -= snowShadowMask * 0.5;
            ...
            #if _SNOWPARTICULATE
               // compute mask
               fixed snowMask = ComputeParticulateMask(wuv, _SnowParticulateParams) * _SnowParticulateStrength;
               snowMask *= snowWeight;
...
ENDCG
   ...
   Dependency "BaseMapShader" = "Hidden/MicroSplat/Example_Snow_Base-1767377448"
